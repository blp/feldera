...
use crate::storage::file::cache::FileCacheEntry;
...
pub struct AsyncCacheContext {
    ...
    requests: Mutex<
        BTreeMap<
            ...
            (
                Option<Compression>,
                Vec<oneshot::Sender<Result<FileCacheEntry, Error>>>,
            ),
        >,
    >,
...
impl AsyncCacheContext {
    pub fn new(cache: Arc<BufferCache<FileCacheEntry>>, file: &dyn FileReader) -> Self {
        Self {
            cache,
