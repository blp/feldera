...
pub enum FetchIndexedZSet<'a, 'b, K0, A0, K1, A1>
where
    K0: DataTrait + ?Sized,
    A0: DataTrait + ?Sized,
    K1: DataTrait + ?Sized,
    ...
    A1: WeightTrait + ?Sized,
{
    pub fn new(
        ...
        Ok(Self::Column0(Some(Fetch0::new(reader, keys)?)))
    }
    ...
    pub fn is_done(&self) -> bool {
        match self {
            FetchIndexedZSet::Column0(option0) => option0.as_ref().unwrap().is_done(),
            FetchIndexedZSet::Column1(multifetch1) => multifetch1.is_done(),
        }
    }
    ...
    pub fn results(
        ...
        match self {
            FetchIndexedZSet::Column0(_) => {
                panic!("can't get results because FetchIndexedZSet is not done yet")
            }
            FetchIndexedZSet::Column1(multifetch1) => multifetch1.results(factories),
        }
    }
    ...
    pub async fn async_results(
        ...
        let multifetch1 = match self {
            FetchIndexedZSet::Column0(option0) => {
                let mut inner0 = option0.unwrap();
                inner0.async_run().await?;
                inner0.next_column()?
            }
            FetchIndexedZSet::Column1(multifetch1) => multifetch1,
        };
        multifetch1.async_results(factories).await
    }
}

pub struct Fetch0<'a, 'b, K, A, N, T>
where
    K: DataTrait + ?Sized,
    A: DataTrait + ?Sized,
{
...
where
    ...
    T: ColumnSpec,
{
    pub fn next_column(self) -> Result<Fetch1<'a, K, NK, NA, T>, Error> {
        Fetch1::new(self)
    }
}

pub struct Fetch1<'a, K0, K1, A1, T>
where
    K0: DataTrait + ?Sized,
    K1: DataTrait + ?Sized,
    A1: WeightTrait + ?Sized,
