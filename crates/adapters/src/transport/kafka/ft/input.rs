        ...
        for (topic, partition_offsets) in topics.iter().zip(initial_offsets) {
            ...
            for (partition, initial_offset) in (0..).zip(partition_offsets) {
                let mut queue = self
                    ...
                    .ok_or(anyhow!(
                        "could not split partition {partition} from {topic}"
                    ))?;
            ...
            match message {
                Err(KafkaError::PartitionEOF(p)) if p >= 0 => {
                    ...
                    let topics = partition_counts
                        ...
                        .collect::<Vec<_>>();
                    if topics.len() == 1 {
                        receivers[topics[0]][p].eof.store(true, Ordering::Relaxed);
                    } else if topics.len() > 1 {
                        return Err(anyhow!("received EOF on ambiguous partition"));
                    }
                }
                Err(e) => {
        ...
        while let Some((metadata, ())) = command_receiver.blocking_recv_replay()? {
            ...
            loop {
                ...
                match self.kafka_consumer.poll(Duration::ZERO) {
                    ...
                    Some(Ok(message)) => {
                        return Err(anyhow!("All partitions are split but the main consumer received a message anyway: {message:?}"));
                    }
                    None => (),
                }
                ...
                match command {
                    ...
                    InputReaderCommand::Queue => {
                        ...
                        let offsets = topics
                            ...
                            .collect();
                        println!("writing log record {offsets:#?}");
                        consumer.extended(
                            total,
                            hasher.finish(),
...
impl PartitionReceiver {
    ...
    fn run(
        ...
        if next_offset > max_offset {
            println!("next_offset {next_offset} is beyond max offset {max_offset}");
            return false;
        }
